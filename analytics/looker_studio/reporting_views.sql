...
--
-- Why views:
--   * Keep business logic (date bucketing, age bands, discount) in one
--     version-controlled place instead of scattered Looker calculated fields.
--   * Control query cost — charts scan narrow, typed columns.
--   * Keep `store_id` as a column so a Looker `@storeId` parameter can filter
--     to one tenant (see BUILD_GUIDE.md). Privacy: raw userId / userName are
--     NOT exposed — customers appear only as a salted-free SHA256 `user_key`
--     plus aggregate gender / age band.
--
-- Grain:
...
--       (or paste each statement into the BigQuery Studio console)
-- ============================================================================
...
WITH orders AS (
  SELECT
    ...
    END AS age,
    ROW_NUMBER() OVER (
      PARTITION BY storeId, userId ORDER BY createdDate
...
SELECT
  ...
  unitDiscounted                                     AS unit_discounted,
  totalPrice                                         AS line_revenue
