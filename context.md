...
- ~~**Sale analytics carry client-authored totals**~~ — closed 2026-09-08: both BigQuery ETL triggers are now `onDocumentWritten`, so a sale is re-exported when `createPaymentPreference` replaces its money fields. The tables became append-only revision history; the Looker views keep the newest revision per `saleDocumentId`. **`analytics/looker_studio/reporting_views.sql` still has to be re-run in BigQuery.**
